...
### Что менялось в ветке, которая уже слита с dev
  1. Делаем checkout нужной ветки из origin
  2. Делаем checkout в dev 
  3. Открываем историю dev и ищем коммит до слияния нашей ветки и dev. 
  4. Делаем checkout в этот commit и сравниваем две ветки стоя в dev.
  5. Если используем IDE, напрмер Visual Studio, то в режиме сравнения можно нажать Switch Branch
...
git branch -a --contains <commit hash>                                       # Какие ветки содержат коммит. Если коммит вмержен в другие ветки, то эти ветки так же будут показаны 
git log c0118fa..HEAD --ancestry-path --merges | tail -n 1                   # В какую ветку изначально был сделан коммит